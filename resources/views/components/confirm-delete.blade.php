@props(['modalId', 'title', 'url'])

<button type="button" data-modal-target="{{ $modalId }}" data-modal-toggle="{{ $modalId }}">
  <i class="fas fa-trash-can cursor-pointer text-lg text-red-600 hover:text-red-800"></i>
</button>
...
<div
  ...
  id="{{ $modalId }}" data-modal-backdrop="static">
    ...
    <div class="relative rounded-lg bg-white shadow dark:bg-slate-800">
      ...
      <div class="p-6 pt-0 text-center">
        <form action="{{ $url }}" method="POST">
          ...
          <h3 class="mb-6 mt-5 text-lg text-slate-700 dark:text-slate-400">
            Yakin Menghapus <strong>{{ $title }}?</strong>
          </h3>
          <x-danger-button class="mr-2 px-3 py-2.5">
            Ya, Hapus
          </x-danger-button>
          <button type="button"
            class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-center text-base font-medium text-slate-900 hover:bg-slate-100 focus:ring-4 focus:ring-primary-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-slate-700"
            data-modal-hide="{{ $modalId }}">
            Batal
          </button>
        </form>
