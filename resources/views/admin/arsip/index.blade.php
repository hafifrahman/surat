            ...
            <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <span class="ml-1 text-slate-400 dark:text-slate-500 md:ml-2" aria-current="page">Arsip</span>
                </div>
              </li>
            </ol>
          </nav>
          <h1 class="text-xl font-semibold text-slate-900 dark:text-white sm:text-2xl">Daftar arsip</h1>
        </div>
        <div class="sm:flex">
          <div class="mb-3 block items-center dark:divide-slate-700 sm:mb-0 sm:divide-x sm:divide-slate-100">
            <div class="relative mt-1 lg:w-64 xl:w-96">
              <form class="flex items-center lg:pr-3" action="/{{ currentRole() }}/arsip" method="GET">
                <x-input type="search" name="q" class="text-sm" value="{{ $search }}"
                  placeholder="Cari arsip..." autocomplete="off" />
                <button
          ...
          <div class="ml-auto flex items-center space-x-2 sm:space-x-3">
            @include('admin.arsip.partial.add')
            <a href="/{{ currentRole() }}/arsip/export/pdf"
                ...
                @forelse ($arsips as $arsip)
                  <tr>
                    ...
                    <td
                      class="max-w-xs overflow-hidden truncate px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300">
                      @if ($arsip->upload)
                        <x-modal-detail-attachment id="detail-arsip-attachment-{{ $arsip->id_arsip }}"
                          src="{{ asset('storage/uploads/arsip/' . $arsip->upload) }}" />
                      @else
                    ...
                    <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-slate-900 dark:text-slate-300">
                      {{ $arsip->tgl_arsip }}</td>
                    <td class="space-x-1 whitespace-nowrap px-4 py-3">
                      @include('admin.arsip.partial.edit')
                      <x-confirm-delete modalId="delete-arsip-modal-{{ $arsip->id }}" title="arsip"
                        :url="route(currentRole() . '.arsip.destroy', $arsip)" />
                      <a
                        @if ($arsip->upload) href="{{ route(currentRole() . '.arsip.download', $arsip->upload) }}" @else @endif>
                        <i
                          class="fa-solid fa-download text-lg text-green-600 hover:text-green-800 dark:text-green-500 dark:hover:text-green-600"></i>
                      </a>
                    </td>
                  </tr>
                @empty
