            ...
            <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <span class="ml-1 text-slate-400 dark:text-slate-500 md:ml-2" aria-current="page">Surat Keluar</span>
                </div>
              </li>
            </ol>
          </nav>
          <h1 class="text-xl font-semibold text-slate-900 dark:text-white sm:text-2xl">Daftar Surat Keluar</h1>
        </div>
        <div class="sm:flex">
          <div class="mb-3 block items-center dark:divide-slate-700 sm:mb-0 sm:divide-x sm:divide-slate-100">
            <div class="relative mt-1 lg:w-64 xl:w-96">
              <form class="flex items-center lg:pr-3" action="{{ route(currentRole() . '.surat-keluar.index') }}"
                method="GET">
          ...
          <div class="ml-auto flex items-center space-x-2 sm:space-x-3">
            @include('admin.surat-keluar.partial.add')
            <a href="/{{ currentRole() }}/surat-keluar/export/pdf"
                ...
                @forelse ($suratKeluar as $surat)
                  <tr>
                    ...
                    <td
                      class="max-w-sm overflow-hidden truncate px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 xl:max-w-xs">
                      @if ($surat->upload)
                        <x-modal-detail-attachment id="detail-sk-attachment-{{ $surat->id_sk }}"
                          src="{{ asset('storage/uploads/surat-keluar/' . $surat->upload) }}" />
                      @else
                    ...
                    <td class="space-x-1 whitespace-nowrap px-4 py-3">
                      @include('admin.surat-keluar.partial.edit')
                      <x-confirm-delete modalId="delete-sk-modal-{{ $surat->id_sk }}" title="Surat" :url="route(currentRole() . '.surat-keluar.destroy', $surat)" />
                      <a
                        @if ($surat->upload) href="{{ route(currentRole() . '.surat-keluar.download', $surat->upload) }}" @else @endif>
                        <i
                          class="fas fa-download text-lg text-green-600 hover:text-green-800 dark:text-green-500 dark:hover:text-green-600"></i>
                      </a>
                    </td>
                  </tr>
                @empty
